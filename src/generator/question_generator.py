...
class QuestionGenerator:
    ...
    def _retry_and_parse(self, prompt, parser, topic, difficulty):
        ...
        for attempt in range(settings.MAX_RETRIES):
            ...
            try:
                ...
                self.logger.info(
                    "Successfully parsed the question"
                )
    ...
    def generate_mcq(
        ...
        try:
            ...
            self.logger.info(
                "Generated a valid MCQ Question"
            )
            ...
            self.logger.error(
                f"Failed to generate MCQ : {str(e)}"
            )
    ...
    def generate_fill_blank(
        ...
        try:
            ...
            parser = PydanticOutputParser(
                pydantic_object=FillBlankQuestion
            )
            ...
            self.logger.info(
                "Generated a valid Fill in Blanks Question"
            )
            ...
            self.logger.error(
                f"Failed to generate fillups : {str(e)}"
            )

            raise CustomException(
                "Fill in blanks generation failed",
                e
            )